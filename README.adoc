...
This project contains Drools & jBPM OSGi examples that you can deploy on supported release https://www.jboss.org/products/fuse.html[RedHat JBoss Fuse 6.2]
...
- As JBoss Fuse 6.2 has not been yet released, we must download it from the Early Access build
- Open a browser at the following address and select one of the https://repository.jboss.org/nexus/content/groups/ea/org/jboss/fuse/jboss-fuse-full/[latest release]
- Unzip it under your local temp directory *~/temp*
- Change to the JBoss Fuse bin directory where +xxx+ should be changed according to the release downloaded

  cd ~/temp/jboss-fuse-6.2.0.redhat-xxx/bin
...
To deploy this module on the container, use this JBoss Fuse command which will install `decision-table` module and the example.
When the bundle will be started, this information will appear in the Console

    @root> features:install simple-decisiontable-rule
    KieSession created.
...
As Rules will be designed by business analysts in a company, they will prefer to use a XLS Spreadsheet document that they will provide
 to the project (developer, ...) as artifact to be used by the application to calculate decision. In the previous example, the decision table
...
to point to your XLS file on the file system, before to build the project and deploy on itJBoss Fuse.

Project can be deployed using the following command :
...
jbpmn process is also supported and can be deployed too on JBoss Fuse. Like the Drools Simple Rule Example, you should install a feature
...
- Start JBoss Fuse with clean option to remove previously installed bundles and start the server with the features defined by default under the file
...
3. Drools project (kie + rules) started using CDI and Weld
  ...
  Remark : Check that javax.annotation line if well uncommented within the file etc/jre.properties for your JDK
...
REMARK : Does not work anymore on JBoss Fuse 6.1 as Camel 2.14 depends on Blueprint 1.1
